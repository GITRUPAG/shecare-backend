...
public class UserService {
    ...
    public AuthResponse login(String identifier, String password) {
        ...
        return new AuthResponse(token, name, user.getRole().name(), user.getUsername(), user.getEmail());
    }
    ...
    public AuthResponse googleLogin(String email, String name, String googleId, String picture) {
        ...
        return new AuthResponse(token, fullName, user.getRole().name(), user.getUsername(), user.getEmail());
    }
    ...
    public AuthResponse otpLogin(String phone) {

    User user = userRepository.findByPhoneNumber(phone)
            .orElseGet(() -> createOtpUser(phone));

    String token = jwtService.generateToken(
            user.getEmail() != null ? user.getEmail() : phone,
            user.getRole().name()
    );
...
private User createOtpUser(String phone) {
    ...
    // Generate username
    String baseUsername = "user" + phone.substring(phone.length() - 4);
    user.setUsername(generateUniqueUsername(baseUsername));

    user.setPassword(null); // 🔥 no password initially
